...
- [[#backup][BACKUP]]
- [[#code-comment-header-macros][CODE COMMENT HEADER MACROS]]
- [[#company][COMPANY]]
- [[#dashboard][DASHBOARD]]
- [[#diminish][DIMINISH]]
- [[#elfeed][ELFEED]]
...
- [[#tuareg-ocaml-formatting][TUAREG OCAML FORMATTING]]
- [[#which-key][WHICH-KEY]]
...
* CODE COMMENT HEADER MACROS
Quick shortcuts for inserting formatted comment headers in source files.
`C-c a` creates an equals-sign header, while `C-c b` creates a dashed header.
The cursor is placed in the middle line so the section title can be typed immediately.
#+begin_src emacs-lisp
(require 'newcomment)
(require 'subr-x)

(defun cole/insert-comment-header ()
  "Insert a 79-character comment header using the current language's comments."
  (interactive)

  ;; Make sure comment-start/comment-end are set for the current major mode.
  ...
  (let* ((start (string-trim (or comment-start "")))
    ...
    ;; Middle line -- leave point here for typing the title.
    (insert start " ")

    ;; Insert everything after point without moving point.
    (save-excursion
      (when (not (string-empty-p end))
        (insert " " end))
...
(global-set-key (kbd "C-c a") #'cole/insert-comment-header)
#+end_src
...
* ERROR FIXES
Stops lisp lexical binding warning
#+begin_src emacs-lisp
;;; config.el --- Cole's Emacs config -*- lexical-binding: t; -*-
#+end_src
